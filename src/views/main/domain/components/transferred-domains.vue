  ...
  <div class="app-container px-0">
    ...
    <div class="wrap">
      ...
      >
        ...
        <el-table-column prop="name" label="Domain Name" min-width="200">
          <template slot-scope="scope">
        ...
        <el-table-column prop="created_at" label="Date" min-width="200">
          <template slot-scope="scope">
            <span class="py-3 d-block position-relative">
              {{ formatDate(scope.row["created_at"]) || "---" }}
              {{ moment(scope.row["created_at"]).format("HH:mm A") || "---" }}
            </span>
          </template>
        </el-table-column>
            ...
            >
              ...
              <el-dropdown-menu slot="dropdown" style="width: 180px">
                <li
                  ...
                  @click="VerifyTransfer(scope.row)"
                >
                  Verify Transfer
                </li>
              </el-dropdown-menu>
    ...
    <div>
      <el-dialog
        title="Re-Verify Your Domain Transfer"
      ...
      >
        <el-form
          ref="createPrcForm"
          :model="createPrc"
          @submit.native.prevent="handleReVerify"
        >
              ...
              <el-col>
                <div class="" style="width: fit-content; margin-left: auto">
                  ...
                  <el-button
                    ...
                    @click.native.prevent="handleReVerify"
                    >Transfer</el-button
                  >
                </div>
              </el-col>
...
export default {
  ...
  mounted() {
    // this.fetchTLDLists();
    this.fetchTrfDomains();
  },
  watch: {
    currentPage: "fetchTrfDomains",
  },
  methods: {
    moment,
    handleReVerify() {
      ...
      const payload = {
        domains: [
          {
            auth_key: this.createPrc.auth_key,
            sld: this.createPrc.sld,
      ...
      axios
        ...
        .catch((error) => {
          ...
          if (errorRes.status >= 400 || errorRes.status === false) {
            ...
            console.error("Error during domain transfer:", error);
            this.$message.error("An error occurred during domain transfer.");
          }
          this.loading = false;
        })
        .finally(() => {
      ...
      axios
        ...
        .catch((error) => {
          ...
          if (errorRes.status >= 400 || errorRes.status === false) {
            ...
            this.$message.error(
              "Error occurred while fetching tlds. Please try again later."
            );
          }
        });
    },
    ...
    fetchTrfDomains() {
      ...
      axios
        .get(this.dynamic_base_url("admin/domains/transferred"), {
          headers: { Authorization: "Bearer " + getToken() },
          params,
        })
        .then((response) => {
    ...
    VerifyTransfer(row) {
      ...
      if (parts.length > 1 && parts.length < 3) {
        this.createPrc.sld = parts[0];
        this.createPrc.tld_id = row.tld_id;
        this.createPrc.tld_name = `.${parts.slice(1)}`;
      } else if (parts.length > 2) {
        this.createPrc.sld = parts[0];
        this.createPrc.tld_id = row.tld_id;
        this.createPrc.tld_name = `.${parts.slice(1, 2)}.${parts.slice(2)}`;
      } else {
        this.createPrc.sld = row.sld;
        this.createPrc.tld_id = row.tld_id;
      }
